		...
		if (!response.error) {
			...
			for (const product of cart) {
				await axios.post(`http://localhost:5021/booking/create/${product.id}`, {
					...
					discountName: product?.discount?.code ?? null,
					appliedDiscount: product?.discount?.percentage ?? null,
		...
		<form onSubmit={onSubmit}>
			...
			<div className="p-6 w-[700px] rounded-lg mt-8 flex justify-between" style={{ boxShadow: "0px 1px 4px 2px rgba(0, 0, 0, 0.40)" }}>
				...
				<div className="flex flex-grow justify-center text-center">
					<div>
						<p className="text-2xl font-bold mb-2 text-[#EB4710]">S${(cart.reduce((a, p) => a +(((p.adultQuantity * p.activity.adultPrice) + (p.childQuantity * p.activity.childPrice)) * (p.discount ? (1 - p.discount.percentage / 100) : 1)), 0)).toFixed(2)}</p>
