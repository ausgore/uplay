...
const RatingButton = ({ rating, btnRating, setRating }) => {
	return <button className={`${rating == btnRating ? "border-[#ed8a19] bg-white" : "border-[#CFCFCF] bg-[#CFCFCF]"} border-2 px-4 py-[5.5px] rounded-lg outline-none w-fit`} style={{ boxShadow: "0px 4px 4px 0px rgba(0, 0, 0, 0.25)" }} onClick={() => {
		...
		<div className="flex flex-row items-center">
			...
			<p className={`text-sm font-semibold ${rating == btnRating ? "text-[#ed8a19]" : ""}`}>{btnRating}</p>
		</div>
	</button>
}
