...
const Home = () => {
	...
	return (
		<>
			<Nav transparent={true} disableSearch />
			<div className={`h-[850px] w-full transition-opacity duration-1000 ease-in-out ${fade ? "opacity-100" : "opacity-0"} flex justify-center items-center`} style={{ backgroundRepeat: "no-repeat", backgroundSize: "cover", backgroundPosition: "bottom", backgroundImage: `url(${images[currentIndex]})`, boxShadow: "0px 4px 4px 0px rgba(0, 0, 0, 0.25)" }}>
				<div>
					...
					<div className="relative items-center flex">
						...
						<button onClick={() => navigate(`/activities?search=${search}`)} className="absolute right-4 text-white font-semibold px-5 py-2 rounded-lg" style={{ background: "linear-gradient(115deg, #933EFF -3.79%, #BA93FF 107.45%)", boxShadow: "0px 4px 4px 0px rgba(0, 0, 0, 0.25)" }}>
							Search
						</button>
					</div>
				</div>
			...
			<div className="max-w-[1400px] mx-auto px-4 pt-32 mb-20">
				...
				<div className="grid grid-cols-4 gap-16 gap-y-14">
					{activities?.sort((a, b) => {
						const average = (b.reviews.reduce((x, r) => x + r.rating, 0) / b.reviews.length) - (a.reviews.reduce((x, r) => x + r.rating, 0) / a.reviews.length);
						if (average !== 0) return average;
						else return b.reviews.length - a.reviews.length;
					}).map(activity => <ActivityCard key={activity.id} activity={activity} />).splice(0, 8)}
				</div>
			</div>
		</>
	);
